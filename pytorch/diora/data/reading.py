...
class NLIReader(object):
    ...
    def read_line(self, line):
        ...
        # Added now
        parse_1 = example['sentence1_binary_parse']
        parse_2 = example['sentence2_binary_parse']
        return dict(s1=s1, label=label, s2=s2, t1=t1, t2=t2, example_id=example_id, spans_1=spans_1, spans_2=spans_2, \
                    parse_1=parse_1, parse_2=parse_2)
        # Added now
        # return dict(s1=s1, label=label, s2=s2, t1=t1, t2=t2, example_id=example_id, spans_1=spans_1, spans_2=spans_2)
    ...
    def read_sentences(self, filename):
        ...
        with open(filename) as f:
            for line in tqdm(f, desc='read'):
                ...
                # Added now
                parse_tree1, parse_tree2 = smap["parse_1"], smap["parse_2"]
                # Added now
                ...
                if not skip_s2:
                    ...
                    # Added now
                    parse_tree.append(parse_tree2)
                    # Added now

        extra['example_ids'] = example_ids
        # Added now
        extra["parse_tree"] = parse_tree
        # Added now
    ...
    def read(self, filename):
        ...
        with open(filename) as f:
            for line in tqdm(f, desc='read'):
                ...
                # Added now
                tree.append(data['tree'])
                # Added now
                tmp_span = []
                for s in data['span_pos']:
                    if len(s) > 1:
                        tmp_span.append([s[0], len(s)])
                spans.append(tmp_span)
